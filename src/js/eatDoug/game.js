...
export default class Game {
    ...
    constructor(option) {
        ...
        //回调函数的名称：getScore  获得分数后的回调函数，会将当前的分数传入
        ...
        this.hasEat = 0; //已经吃的食物个数
        this.footNum = 16; //食物的总个数
        ...
        this.drawMaze();
        this.cellWidth = this.width / 21;       //每个单元的宽度
        this.cellHeight = this.height / 21;     //每个单元的高度
        ...
        Promise.all([this.wall.render(-550, -550), this.space.render(-550, -550), this.foot.render(-550, -550)], this.pathSpot.render(-550, -550)).then(() => {
            this.render();
            this.isStart = true;
    ...
    drawMaze() {
        ...
        this.hasEat = 0;
        this.addFoot(); //添加食物
        //判断用户是否传入自动游戏以及当前游戏是否准备就绪
        if(this.auto && this.isStart){
            this.autoPlay();
        }
    }
    start() {
    ...
    playerMove(newPosX, newPosY) {
        ...
        if (this.getScore(newPosX, newPosY)) {
            this.score += 10
            this.cb.getScore && this.cb.getScore(this.score);
            //调用所传入的getScore回调
        ...
        if (this.arriveExit(newPosX, newPosY) && this.eatAllFoot()) {
            ...
            this.level++;
            //运行所传入的pass回调
            this.cb.pass && this.cb.pass(this.level);
        }
        //如果没有吃完桃子就进入出口则弹出提示
        if (this.arriveExit(newPosX, newPosY) && !this.eatAllFoot()) {
            console.log(this.hasEat);
            alert("桃子都没吃完就想走？？");
            return;
        }
     ...
     */
    getFootPath() {
        if (this.hasEat == this.footNum) return;
        let result = [];
     ...
     */
    autoEatFoot(cb) {
        if (this.hasEat == this.footNum) return cb();
